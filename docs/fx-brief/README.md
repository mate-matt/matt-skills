...
| Profile card | `profile-card` | X profile card for creator/business-card style sharing. |
...
| Raw data export | `json` | Raw FxEmbed JSON, or fxbrief's normalized local JSON model. |
...
Use `fxbrief` `0.2.4` or newer. That version contains the core workflows used by the skill, profile-card rendering, hidden-by-default post-mobile Subscribe buttons, and raw FxEmbed JSON export.
...
Render a profile card with one latest profile post appended:
...
Render a profile card with a longer profile timeline appended:
...
Fetch raw FxEmbed JSON without rendering:

```bash
fxbrief json "https://x.com/user/status/123"
```

Write normalized thread JSON:
...
| --- |
| Use `$matt-fx-brief` to turn `https://x.com/mate_mattt` into a creator profile card with one latest post. |
| <img src="assets/mate-mattt-profile-latest.png" alt="Matt profile card generated by FxBrief" width="430"> |
...
Use $matt-fx-brief to turn this X profile into a creator card with one latest post: https://x.com/mate_mattt
```
...
For JSON export, use `--kind post|thread|quotes`, `--normalized`, `--compact`, and `--out <path>`. The `json` command does not use Playwright or download media.

For profile cards, `--count <1-6>` appends profile posts below the card. It is `0` by default. `--latest-post` is a shortcut for `--count 1`; use `--with-replies` if replies should be eligible. FxEmbed currently exposes profile statuses but not a reliable pinned-post marker.

## Skill Routing Notes

Use one skill, `matt-fx-brief`, for all X/FxEmbed work. A profile URL or handle routes to `profile-card`. A status URL routes by intent: post screenshot requests use `post-mobile` or `post-clean`; X Article archive requests use `article-md`; long screenshot or slice requests use `article-shot`; data inspection requests use `json`. If the user provides only a status URL and the Article/post distinction matters, inspect the payload with `fxbrief json "<url>" --normalized` before choosing the final command.
